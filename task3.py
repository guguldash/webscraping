...
def group_by_decade(movies):
    ...
    for value in movies:
        for year in value:
            if year == "year" and (value[year]) not in unique_year:
    ...
    movie_dic = {vlu2: [] for vlu2 in decate_movie_list}
    for vlu3, value_ in movie_dic.items():
        dec10=int(vlu3)+9
        for movie in movies:
            for yr in movie:
                if (
                    yr == "year"
                    and (int(movie[yr])) <= dec10
                    and (int(movie[yr])) >= vlu3
                ):
                    value_.append(movie)
    import json
